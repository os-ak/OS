...
/*		if(task->h_data.flg==0){
		ide_read(in1,in2,in3);
		}else if(task->h_data.flg==1){
...
void end_hdd(){
	...
	/*	if(taskctl->wait0[0]->h_data.flg==0){
		//	ide_read(in1,in2,in3);
		}else if(taskctl->wait0[0]->h_data.flg==1){
		//	ide_read(in1,in2,in3);
	}*/
		start_hdd(taskctl->wait0[i]->h_data.flg,taskctl->wait0[i]->h_data.in1,taskctl->wait0[i]->h_data.in2,taskctl->wait0[i]->h_data.in3);
	}else if(taskctl->wait_num == 1){
		taskctl->wait_num--;
	  task_run(taskctl->wait0[0],-1,-1);
	}
	return;
}
